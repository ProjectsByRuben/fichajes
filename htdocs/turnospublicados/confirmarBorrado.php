...
<div class="container mt-5">
    <?php
        echo "<h1>ID Turno: </h1><h2>".$_REQUEST["turno"]."</h2>";
        echo "<h1>Fecha: </h1><h2>".$_REQUEST["fecha"]."</h2><br>";
    ?>
...
<script>
    function confirmarBorrado() {
        var turno = "<?php echo $_REQUEST['turno']; ?>";
        
        Swal.fire({
            title: '¿Estás seguro de dar de baja el Turno?',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                ...
                Swal.fire(
                    ...
                    window.location.href = 'borraTurno.php?id=' + encodeURIComponent(turno);
                });
            } else if (result.dismiss === Swal.DismissReason.cancel) {
                // Acción a realizar si el usuario deniega (por ejemplo, mostrar un mensaje de cancelación)
                Swal.fire(
                    'Acción cancelada',
                    'El Turno no ha sido dado de baja.',
                    'info'
                );
            }
        });
